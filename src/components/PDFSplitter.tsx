...
export const PDFSplitter = () => {
  ...
  const handleShareResult = async (url: string, name: string, index: number) => {
    setIsSharing(prev => ({ ...prev, [index]: true }));
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'PDF Divisé',
          text: 'Voici un extrait de mon PDF via PDF Master',
        });
      } else {
        await navigator.share({
          title: 'PDF Divisé',
          text: `Lien vers le document : ${name}`,
      ...
      setIsSharing(prev => ({ ...prev, [index]: false }));
    }
  };
    ...
    <div className="space-y-6">
      ...
      {!file ? (
        <motion.div
          ...
          className={cn('relative group flex flex-col items-center justify-center gap-6 rounded-[2.5rem] border-2 border-dashed p-16 cursor-pointer transition-all',
            isDragging ? 'border-indigo-600 bg-indigo-50' : 'border-indigo-200 hover:border-indigo-500 hover:bg-indigo-50/30')}
        >
          <input 
            type="file" 
            accept=".pdf" 
          ...
          />
          <div className={cn('relative z-10 h-16 w-16 rounded-2xl flex items-center justify-center transition-all pointer-events-none', 
            isDragging ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500 group-hover:bg-indigo-600 group-hover:text-white')}>
            <Scissors size={32} />
          </div>
          <div className="relative z-10 text-center pointer-events-none">
            <p className="text-xl font-black text-slate-900">Diviser un document PDF</p>
            <p className="mt-1 text-sm font-medium text-slate-500">Extrayez des pages ou divisez en plusieurs fichiers</p>
          </div>
        </motion.div>
      ) : (
              ...
              {results.map((r, i) => (
                <a key={i} href={r.url} download={r.name}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={(e) => { e.preventDefault(); handleShareResult(r.url, r.name, i); }}
                      disabled={isSharing[i]}
                      className="p-1.5 hover:bg-violet-50 text-violet-600 rounded-lg transition-colors disabled:opacity-50"
                      title="Partager"
                    >
                      {isSharing[i] ? <Loader2 size={14} className="animate-spin" /> : <Share2 size={14} />}
                    </button>
                    <a href={r.url} download={r.name}
                      className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 group-hover:translate-x-0.5 transition-transform">
