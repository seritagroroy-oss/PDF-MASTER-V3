...
export const PDFConverter: React.FC<PDFConverterProps> = ({
  ...
  const handleShare = async () => {
    if (!resultUrl || !rawFiles[0]) return;
    setIsSharing(true);
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Fichier converti',
          text: 'Voici mon fichier converti avec PDF Master',
        });
      } else {
        await navigator.share({
          title: 'Fichier converti',
        ...
        >
          ...
          {isProcessing && (
            ...
            >
              ...
              >
                <div
                  className="absolute top-0 left-0 h-full bg-gradient-to-r from-indigo-500 to-indigo-600 transition-[width] duration-300 ease-out"
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                <button
                  ...
                  className="group relative flex-1 flex items-center justify-center gap-3 px-10 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold shadow-lg shadow-emerald-200 transition-all hover:scale-105 active:scale-95 overflow-hidden"
                >
                ...
                <button
                  onClick={handleShare}
                  disabled={isSharing}
                  className="flex-1 flex items-center justify-center gap-3 px-10 py-4 bg-violet-600 hover:bg-violet-700 text-white rounded-2xl font-bold shadow-lg shadow-violet-200 transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                >
