    ...
    try {
      ...
      for (let i = 1; i <= Math.min(totalPages, 10); i++) { // Limit to 10 pages to avoid freezing
        ...
        const result = await Tesseract.recognize(imgData, language, {
          logger: m => {
            if (m.status === 'recognizing text') setProgress(prev => Math.max(prev, Math.round(m.progress * 100)));
          }
        });
              ...
              <div className="relative group">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                  <button onClick={handleShare} disabled={isSharing}
                    className="p-2 bg-violet-50 text-violet-600 rounded-xl hover:bg-violet-100 transition-colors shadow-sm flex items-center justify-center disabled:opacity-50">
                  ...
                  <button onClick={async () => { await navigator.clipboard.writeText(extractedText); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                    className="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition-colors shadow-sm flex items-center justify-center">
