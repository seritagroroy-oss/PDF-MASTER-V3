...
import { Combine, Scissors, Minimize2, Layout, FileImage, Shield, Layers, Clock, TrendingUp, User, LogOut, Settings, MessageSquare, ScanText, Hash, Package, Camera, PenTool, Sparkles } from 'lucide-react';
...
const TOOL_META: Record<string, { icon: React.ElementType; color: string; bg: string; label: string }> = {
  ...
  ocr:      { icon: ScanText,   color: 'text-teal-600',    bg: 'bg-teal-100',    label: 'Scan OCR' },
  scanner:  { icon: Camera,     color: 'text-cyan-600',    bg: 'bg-cyan-100',    label: 'Scanner' },
  sign:     { icon: PenTool,    color: 'text-emerald-600', bg: 'bg-emerald-100', label: 'Signature' },
  purify:   { icon: Sparkles,   color: 'text-violet-600',  bg: 'bg-violet-100',  label: 'Nettoyage' },
