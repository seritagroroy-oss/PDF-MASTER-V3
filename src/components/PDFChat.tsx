    ...
    try {
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Document PDF',
          text: `Voici le document : ${file.name}`,
        });
      } else {
        await navigator.share({
          title: 'Document PDF',
        ...
        <div className="flex items-center gap-2">
          ...
          {file && !isExtracting && (
            <button 
              ...
              disabled={isSharing}
              title="Partager"
              className="p-2 rounded-xl bg-white/10 hover:bg-white/20 transition-all text-white/70 disabled:opacity-50"
            >
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-50/50">
          {!file && !isExtracting && (
            <motion.div
              ...
              className={cn('relative group w-full max-w-md flex flex-col items-center justify-center gap-6 rounded-[2rem] border-2 border-dashed p-12 cursor-pointer transition-all',
            ...
            >
              ...
              <div className={cn('relative z-10 h-16 w-16 rounded-2xl flex items-center justify-center transition-all pointer-events-none', 
                isDragging ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-indigo-600 group-hover:text-white')}>
                <Upload size={28} />
              </div>
