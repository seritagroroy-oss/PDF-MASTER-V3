...
import { Type, Image as ImageIcon, Download, Loader2, CheckCircle2, AlertCircle, Layout, Share2, ArrowLeft } from 'lucide-react';
...
export const PDFWatermark: React.FC<PDFWatermarkProps> = ({
  ...
  React.useEffect(() => {
    if (projectId === 'new' && rawFiles.length > 0) {
      console.log('[PDFWatermark] Registering new project...');
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `watermarked_${new Date().getTime()}.pdf`, { type: 'application/pdf' });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'PDF avec Filigrane',
          text: 'Voici mon PDF avec filigrane via PDF Master',
        });
      } else {
        await navigator.share({
          title: 'PDF avec Filigrane',
            ...
            >
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={handleShare}
                  disabled={isSharing}
                  className="flex items-center gap-2 px-8 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-semibold shadow-lg shadow-violet-200 transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                >
