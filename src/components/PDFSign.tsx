...
export const PDFSign: React.FC<PDFSignProps> = ({
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `Document_Signe.pdf`, { type: 'application/pdf' });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Document Signé',
          text: 'Voici mon document signé via PDF Master',
        });
      } else {
        await navigator.share({
          title: 'Document Signé',
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold transition-transform hover:scale-105 shadow-xl shadow-emerald-600/20"
            >
              <Download size={20} />
              Télécharger
            </button>
            <button
              onClick={handleShare}
              disabled={isSharing}
              className="flex items-center justify-center gap-2 px-8 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold transition-transform hover:scale-105 shadow-xl shadow-violet-600/20 disabled:opacity-50"
            >
