...
export const PDFScanner: React.FC = () => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `Scanner_${new Date().getTime()}.pdf`, { type: 'application/pdf' });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Document Scanné',
          text: 'Voici mon document scanné via PDF Master',
        });
      } else {
        await navigator.share({
          title: 'Document Scanné',
      ...
      <AnimatePresence>
        {resultUrl && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[300] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-6"
          >
            <motion.div
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              className="bg-white dark:bg-slate-800 rounded-[3rem] p-8 sm:p-12 max-w-lg w-full text-center shadow-2xl border border-slate-100 dark:border-slate-700"
            >
              <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-emerald-500/10">
                <CheckCircle2 size={40} />
              </div>
              <h3 className="text-3xl font-display font-black text-slate-900 dark:text-white mb-2">Scan Terminé !</h3>
              <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium">
                Votre document de {scannedImages.length} page(s) a été converti en PDF avec succès.
              </p>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex gap-3">
                  <a
                    href={resultUrl}
                    download={`Scanner_${new Date().getTime()}.pdf`}
                    className="flex-1 flex items-center justify-center gap-2 py-4 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition-all hover:scale-[1.02] shadow-xl shadow-emerald-600/20"
                  >
                    <Download size={20} />
                    Télécharger
                  </a>
                  <button
                    onClick={handleShare}
                    disabled={isSharing}
                    className="flex-1 flex items-center justify-center gap-2 py-4 bg-violet-600 text-white font-bold rounded-2xl hover:bg-violet-700 transition-all hover:scale-[1.02] shadow-xl shadow-violet-600/20 disabled:opacity-50"
                  >
