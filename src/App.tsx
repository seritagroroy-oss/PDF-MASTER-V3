...
const PDFPurifier = lazy(() =>
  import('./components/PDFPurifier').then((module) => ({ default: module.PDFPurifier })),
);
const About = lazy(() =>
...
const tools = [
  ...
  },
  {
    ...
    name: 'Nettoyer Scan',
    shortName: 'Nettoyage',
    description: 'Blanchit les scans sombres et améliore le contraste pour impression.',
    detail: 'Idéal pour les documents administratifs mal scannés.',
    icon: Sparkles,
    color: 'bg-indigo-600',
    soft: 'from-orange-500/15 to-amber-500/10',
...
const preloadTool = (tool: Exclude<Tool, 'home' | 'dashboard'>) => {
  switch (tool) {
    ...
    case 'purify': void import('./components/PDFPurifier'); break;
    case 'about': void import('./components/About'); break;
  }
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    document.title = currentTool
      ? `${currentTool.name} | PDF Master | Outil PDF Gratuit et Privé`
      : 'PDF Master | Fusionner, Compresser et Convertir vos PDF en ligne';
  }, [activeTool]);
  ...
  return (
    ...
    >
        ...
        <nav className="sticky top-0 z-50 border-b border-indigo-500/10 bg-slate-900/80 backdrop-blur-xl">
          <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex w-full items-center justify-start xl:justify-center gap-1 xl:gap-2">
                ...
                <div className="relative">
                  <button 
                    ...
                    className={cn(
                      "flex items-center gap-1.5 whitespace-nowrap shrink-0 rounded-full px-3 py-1.5 xl:px-4 xl:py-2 text-xs xl:text-sm font-semibold transition-colors",
                      isPlusOpen ? "bg-slate-800 text-white" : "text-slate-300 hover:bg-slate-900 hover:text-white"
                    )}
                  >
                    Plus <ChevronDown size={14} className={cn("transition-transform duration-300", isPlusOpen && "rotate-180")} />
                  </button>
                          ...
                          {tools.slice(4).map(tool => {
                            ...
                            return (
                              <button
                                ...
                                className={cn("w-full text-left px-3 py-2.5 text-sm font-semibold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center gap-3",
                                  activeTool === tool.id ? "text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-500/10" : "text-slate-700 dark:text-slate-300")}
                              >
                                <span className={cn("p-2 rounded-xl flex shrink-0 items-center justify-center transition-transform", (tool as any).bg || tool.color.replace('text-', 'bg-').replace('600', '100'))}>
                ...
                {/* À propos déplacé en bas de page */}

              </div>
            </div>

            <div className="hidden items-center shrink-0 gap-3 md:flex">
              <button
                type="button"
                onClick={() => setIsWizardOpen(true)}
                className="inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-4 py-2 text-sm font-bold text-indigo-400 transition hover:bg-indigo-500/20 hover:border-indigo-500/40"
              >
                <Sparkles size={16} />
                Besoin d'aide ?
              </button>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-sm font-bold text-emerald-500 transition hover:bg-emerald-500/20 hover:border-emerald-500/40"
              >
                <Users size={16} />
            ...
            <div className="flex items-center gap-1 md:hidden">
              ...
              <button
                type="button"
                onClick={() => setIsWizardOpen(true)}
                className="p-2 text-indigo-400 hover:bg-slate-800 rounded-xl transition-colors"
                title="Besoin d'aide ?"
              >
                <Sparkles size={24} />
              </button>
              <button
                ...
                className="rounded-xl p-2 text-slate-200 transition hover:bg-slate-900"
          ...
          <AnimatePresence>
            {isMenuOpen && (
              ...
              >
                <div className="space-y-4 px-4 py-5 font-display">
                  {/* Bouton Aide Mobile dans le menu */}
                  <button
                    type="button"
                    onClick={() => { setIsWizardOpen(true); setIsMenuOpen(false); }}
                    className="flex w-full items-center gap-3 rounded-2xl bg-indigo-500 px-4 py-4 text-left font-black text-white shadow-lg shadow-indigo-500/20"
                  >
                    <Sparkles size={20} />
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-4 text-left font-black text-emerald-500 shadow-lg shadow-emerald-500/10 transition hover:bg-emerald-500/20"
                  >
                    <Users size={20} />
              ...
              >
                <section className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
                  <div className="space-y-8">
                    ...
                    <div className="space-y-5">
                      <h1 className="max-w-4xl text-4xl font-display font-extrabold leading-tight text-slate-950 dark:text-white sm:text-5xl lg:text-7xl">
                ...
                <section className="mt-10 sm:mt-20">
                  <div className="mb-8 flex items-end justify-between gap-6">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.26em] text-slate-400 dark:text-white">
                        Outils
                      </p>
                      <h2 className="mt-2 text-3xl font-display font-bold text-slate-950 dark:text-white">
                  ...
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 mt-8 lg:mt-12">
                    {tools.map((tool) => (
                      ...
                      >
                        ...
                        <h3 className="text-xl font-display font-bold text-slate-950 dark:text-white">{tool.name}</h3>
                        <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-200">{tool.description}</p>
                        <p className="mt-4 text-sm leading-6 text-slate-500 dark:text-slate-300">{tool.detail}</p>
                        
                        <div className="mt-4 flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg w-fit border border-slate-200 dark:border-slate-700">
                          <AlertCircle size={14} className="text-slate-400" />
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Conseil : {
                            tool.id === 'compress' ? 'Idéal pour WhatsApp' :
                            tool.id === 'ocr' ? 'Convertit les photos en texte' :
                            tool.id === 'merge' ? 'Pratique pour les dossiers' :
        ...
        <footer className="border-t border-slate-100/10 bg-white/5 py-8 backdrop-blur-xl">
          <div className="mx-auto flex flex-col md:flex-row max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 gap-6">
            <div className="flex flex-col items-center md:items-start gap-2">
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-slate-400 dark:text-white">SERI TAGRO ROY INDUSTRIE</p>
              ...
              <div className="flex items-center gap-2 px-2 py-1 bg-white/5 rounded-full border border-white/10">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-400">Économiseur de Data : Traitement Local</span>
              </div>
            </div>
            
            <div className="flex flex-col items-center md:items-end gap-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Soutenir le projet (Côte d'Ivoire)</p>
              <button 
                onClick={() => setIsSupportOpen(true)}
                className="group relative flex items-center gap-3 px-4 py-2 bg-gradient-to-r from-orange-500 via-yellow-500 to-cyan-500 rounded-2xl shadow-lg shadow-orange-500/20 hover:scale-105 active:scale-95 transition-all overflow-hidden"
              >
                <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
                <Heart size={16} className="text-white fill-white" />
                <span className="text-xs font-black text-white uppercase tracking-wider">Faire un don de soutien</span>
              </button>
            </div>

            <div className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-white">
              <button onClick={() => handleToolChange('about')} className="hover:text-indigo-400">À propos</button>
      ...
      <WizardModal
        ...
        onSelectTool={(id) => { handleToolChange(id as Tool); setIsWizardOpen(false); }}
      />
